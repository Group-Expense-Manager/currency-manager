...
class ExchangeRateJobIT(
    ...
    private val exchangeRateRepository: ExchangeRateRepository,
        ...
        should("retry exchange rate job successfully") {
            ...
            // then
            waitTillExchangePlan(exchangeRateJobRepository, exchangeRateJob.id)
            val exchangeRateAfterRetry =
                exchangeRateRepository.getExchangeRate(
                    firstExchangeRateResponse.code,
                    secondExchangeRateResponse.code,
                    localDate,
                )
            exchangeRateAfterRetry.also {
                it.exchangeRate shouldBe firstExchangeRateResponse.rates.first().mid.divide(secondExchangeRateResponse.rates.first().mid)
                it.currencyTo shouldBe secondExchangeRateResponse.code
                it.currencyFrom shouldBe firstExchangeRateResponse.code
                it.forDate shouldBe localDate
                it.validTo shouldNotBeBefore localDate
