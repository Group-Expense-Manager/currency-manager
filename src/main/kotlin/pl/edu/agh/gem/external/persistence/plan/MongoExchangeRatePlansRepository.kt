...
@Repository
@MeteredRepository
open class MongoExchangeRatePlansRepository(
    ...
    override fun findReadyAndDelay(): ExchangeRatePlan? {
        ...
        val update =
            Update()
                .set(
                    ExchangeRatePlanEntity::nextProcessAt.name,
                    clock.instant().plus(exchangeRatePlanProcessorProperties.lockTime),
                )
        ...
        return mongoOperations.findAndModify(
            ...
            update,
            options,
            ExchangeRatePlanEntity::class.java,
        )?.toDomain(clock)
    }
    ...
    override fun delete(
        ...
        val query =
            Query.query(
                Criteria.where(ExchangeRatePlanEntity::id.name).isEqualTo(CompositeKey(currencyFrom, currencyTo)),
            )
    ...
    override fun retry(exchangeRatePlan: ExchangeRatePlan) {
        val query =
            Query.query(
                Criteria.where(
                    ExchangeRatePlanEntity::id.name,
                ).isEqualTo(CompositeKey(exchangeRatePlan.currencyFrom, exchangeRatePlan.currencyTo)),
            )
        val update =
            Update()
                .set(
                    ExchangeRatePlanEntity::nextProcessAt.name,
                    clock.instant().plus(exchangeRatePlanProcessorProperties.retryDelay),
                )
    ...
    override fun deleteNotAllowed(currencyPairs: List<Pair<String, String>>) {
        val query =
            Query.query(
                Criteria.where(ExchangeRatePlanEntity::id.name).nin(
                    currencyPairs.map {
                        CompositeKey(it.first, it.second)
                    },
                ),
            )
        mongoOperations.remove(query, ExchangeRatePlanEntity::class.java)
